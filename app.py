...
def load_data(uploaded, default_path: str = "sedai_execution_report_sample_v4.xlsx"):
    ...
    # If still NA, use today to avoid filter errors
    when = when.fillna(pd.Timestamp.today().normalize())
    df['Month'] = when.dt.month_name()
    df['Year'] = when.dt.year

    # Fiscal Year (FY end-year label) and Quarter mapping (Apr-Jun Q1, Jul-Sep Q2, Oct-Dec Q3, Jan-Mar Q4)
...
# Helper to build multiselect with 'All' behavior
...
with col1:
    ...
    funnel_df = pd.DataFrame({
        'Stage': list(pipe_vals.keys()),
        'Value': list(pipe_vals.values())
    })
...
with col2:
    ...
    if by_inf.empty:
        ...
        fig_bar = px.bar(
            by_inf, x='Inference Type', y='Cost Savings in $',
            text='Cost Savings in $', color_discrete_sequence=[COLORS['primary']],
        )
...
# ------------------------------
# CHARTS ROW 2 – Distribution Pie + Recommendations by Sprint (Bar)
# ------------------------------
...
with col3:
    ...
    fig_pie = px.pie(
        mix_df, names='Category', values='Value', color='Category',
        color_discrete_map=CATEGORY_COLORS, hole=0.35
    )
...
with col4:
    ...
    s_grp = flt.groupby('Sprint', dropna=False).agg(
        Recommendations=('Sprint','count'),
        Savings_USD=('Cost Savings in $','sum')
    ).sort_values('Savings_USD', ascending=False).reset_index()
    if s_grp.empty:
        st.info("No data for selected filters.")
    else:
        fig_sprint = go.Figure()
        fig_sprint.add_trace(go.Bar(
            x=s_grp['Sprint'], y=s_grp['Recommendations'], name='Recommendations',
            marker_color=COLORS['accent'], yaxis='y', text=s_grp['Recommendations'], textposition='outside'
        ))
        fig_sprint.add_trace(go.Scatter(
            x=s_grp['Sprint'], y=s_grp['Savings_USD'], name='Savings ($)',
            mode='lines+markers+text', text=[f"${v:,.0f}" for v in s_grp['Savings_USD']],
            textposition='top center', marker_color=COLORS['primary'], yaxis='y2'
        ))
...
# FOOTER NOTE
# ------------------------------
st.caption("FY definition: Apr 1 to Mar 31 (end-year label, e.g., Apr 2024–Mar 2025 is FY2025). Quarters: Q1=Apr–Jun, Q2=Jul–Sep, Q3=Oct–Dec, Q4=Jan–Mar.")
